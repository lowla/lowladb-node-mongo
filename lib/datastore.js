(function (exports) {
  ...
  function _getCollectionNames() {
    ...
    return new Promise(function(resolve, reject){
      ...
      datastore.config.db.collectionNames(function (err, collectionNames) {
        ...
        for (var col in collectionNames) {
          console.log('_getCollectionNames() found: ' + JSON.stringify(collectionNames));
  ...
  function updateDocumentByOperations(lowlaId, versionPreUpdate, updateOps, force) {
    var datastore = this;
    return new Promise(function(resolve, reject){
      ...
      datastore._getCollection(lowlaId.collectionName).then(
        function (collection) {
          try {
            ...
            if (versionPreUpdate && !force) {
              query._version = versionPreUpdate;
            } else {
              isNew = true;
            }

            //TODO - the below will overwrite an existing version if the client did not send a version up with the update.
      ...
      datastore._getCollection(lowlaId.collectionName).then(
        function (collection) {
          try{
            var query = {_id: lowlaId.id };
            if (versionPreDelete) {
              query._version = versionPreDelete;
            }
